...
?>
    ...
    <?php if($status == 'Activated') : ?>
    	...
    	<?php 
        	$signup_form_html = '<form method="post" id="ultimate_updater"><input type="hidden" name="action" value="update_ultimate_keys" /><input type="hidden" id="ustep" name="step" value="register_customer"/><input type="hidden" id="envato_username" value="'.$envato_username.'" name="envato_username"/><input type="hidden" id="envato_api_key" value="'.$envato_api_key.'" name="envato_api_key"/><input type="hidden" id="ultimate_purchase_code" value="'.$purchase_code.'" name="ultimate_purchase_code" /><table class="form-table"><tr valign="top"><th scope="row">'.__("Email","ultimate").'</th><td> <input type="text" id="ultimate_user_email" value="'.$ultimate_user_email_option.'" name="ultimate_user_email" style="width: 85%;" /><span class="masterUltTooltip dashicons dashicons-editor-help" title="'.__("Enter your email address","ultimate").'"></span></td></tr><tr valign="top"><th scope="row">'.__("Confirm Email","ultimate").'</th><td> <input type="text" id="ultimate_user_email_confirm" value="" name="ultimate_user_email_confirm" style="width: 85%;"/><span class="masterUltTooltip dashicons dashicons-editor-help" title="'.__("Confirm your email address","ultimate").'"></span></td></tr><tr valign="top"><th scope="row"></th><td colspan="2"> <input type="checkbox" id="ultimate_user_receive" value="receive" name="ultimate_user_receive" checked="checked"/><label style="font-size: 13px;color: #666;" for="ultimate_user_receive">'.__("Receive important news &amp; updates on email.","ultimate").'</label></td></tr></table></form>';
        ?>
    	...
    	<div class="inside">
        	<div class="main">
            	<?php if($ultimate_skip == 'false' || $ultimate_skip == '') : ?>
                	...
                	<!--<ul id="signup-steps" class="sf__step-indicator">
                    <li class="step1">
                        <span class="sf__step-count">1</span>
                        <span class="sf__step-title">Register License</span>
                    </li>
                    <li class="step2 active">
                        <span class="sf__step-count">2</span>
                        <span class="sf__step-title">Sign up for Support</span>
                    </li>
                </ul>-->
                <div class="clear"></div>
                    ...
                    <input type="button" id="activate" class="button button-primary updater-act-btn updater-act-btn-align" style="margin-top: 20px; margin-right: 10px;" value="<?php echo __('Sign Up For Support'); ?>" data-process="reactivate">
                    <input type="button" id="skip" class="button updater-act-btn" style="margin-top: 20px; " value="<?php echo __('I\'ve already registered'); ?>"/>
                    <span class="spinner" style="float: none;top: 25px;position: relative;"></span>
                <?php else : ?>
					<div class="main after-regi-text">
						<p>We take pride in providing top notch support through our dedicated help center. But please be sure to check the documentation, video tutorials, FAQs & basic troubleshoot guidelines to ensure which will in most of the cases, help you resolve your issue quickly.</p>
						<p>If you still need help - allow us a temporary secure access <span class="masterUltTooltip dashicons dashicons-editor-help" title="<?php echo __('How does this work?<br/><br/>
When you grant us an access to your website by clicking the button below,<br/>
a unique & very secure access token will be generated for us.<br/> We receive this token on our secure APIs.<br/><br/>
This is in no way related to your password or any confidential login information <br/>
and you may at any time revoke this access which invalidates the token and it will <br/>
no longer be usable.'); ?>"></span> to your website & write us about your issue.</p>
					</div>
                    ...
                    <?php 
						...
						{
							$dev_act_btn_txt = __('Grant Temporary Access to Developers'); 
							$button_id = 'developer-access';
						}
						else
						{
							$dev_act_btn_txt = __('Revoke Access'); 
                    ...
                    <div class="after-regi-btn">
                        ...
                        <?php if($ultimate_user_mail != '' || $ultimate_user_email_option != ''): ?>
                            <a href="https://www.brainstormforce.com/support?access_id=<?php echo base64_encode($envato_username) ?>&access_token=<?php echo get_option('bsf-support-profile-access-token'); ?>&site=<?php echo urlencode(site_url()); ?>&wp_nonce=<?php echo wp_nonce_url(urlencode(site_url())); ?>" class="button button-primary updater-act-btn" target="_blank">
                                ...
                                <?php echo __('Visit our Support Center'); ?>
                            </a>
                        <?php else : ?>
                            <input type="hidden" id="ultimate_next_step" name="ultimate_next_step" value="3"/>
                            <a href="javascript:void(0)" id="after-signup" class="button button-primary updater-act-btn no-email" data-process="reactivate" onclick="signup_for_customer_portal();" target="_blank">
                                ...
                                <?php echo __('Sign Up for Customer Profile'); ?>
                            </a>
                        <?php endif; ?>
                        <!-- <a href="javascript:void(0)" id="confirm-deactivate" class="bsf-card-link" data-process="<?php echo $process; ?>">
                            <span class="bsf-card <?php echo ($button == 'Deregister') ? 'bsfred' : 'bsfgreen'; ?>">
                                <span class="bsf-card-cell"><?php echo $button;?></span>
                            </span>
                        </a>-->
    ...
    <div class="inside">
        ...
        <form method="post" id="ultimate_updater">
            ...
            <!--<input type="hidden" id="ultimate_user_email" value="<?php //echo $ultimate_user_email_option ; ?>" name="ultimate_user_email" />-->
            <table class="form-table">
                <tbody>
                    <tr valign="top">
                        <th scope="row"><?php echo __("Envato Username","ultimate"); ?></th>
                        <td> <input type="text" id="envato_username" value="<?php echo $envato_username; ?>" name="envato_username" style="width: 85%;" <?php echo $readonly ?>/><span class="masterUltTooltip dashicons dashicons-editor-help" title="<?php echo __("Enter your envato username","ultimate"); ?>"></span>
                        </td>
                    </tr>
                    <tr valign="top">
                        <th scope="row"><?php echo __("API Key","ultimate"); ?></th>
                        <td> <input type="password" id="envato_api_key" value="<?php echo $envato_api_key; ?>" name="envato_api_key" style="width: 85%;" <?php echo $readonly ?>/><span class="masterUltTooltip dashicons dashicons-editor-help" title="<?php echo __("Enter your envato API Key","ultimate"); ?>"></span>
                        </td>
                    </tr>
                    <tr valign="top">
                        <th scope="row"><?php echo __("Purchase Code","ultimate"); ?></th>
                        <td> <input type="text" id="ultimate_purchase_code" value="<?php echo $purchase_code; ?>" name="ultimate_purchase_code" style="width: 85%;" <?php echo $readonly ?>/><span class="masterUltTooltip dashicons dashicons-editor-help" title="<?php echo __("Enter your purchase key of Ultimate Addons for Visual Composer","ultimate"); ?>"></span>
                        </td>
                    </tr>
                </tbody>
            </table>
...
confirm_deactivate.click(function(){
	var confirm_html = '<div><h2>Are you sure?</h2><p>This means you can not receive updates with new features and support on this website.</p><p>&nbsp;</p><a href="javascript:void(0)" id="deactivate-plugin" class="button button-primary" onclick="init_activate_process(\'#confirm-deactivate\');">Yes</a>&nbsp;&nbsp;<a href="javascript:void(0)" id="deactivate-plugin-cancel" class="button" onclick="hide_bsf_popup(); return false;">Cancel</a><span id="popup-spinner" class="spinner" style="float: none;"></span></div>';
...
{
	var signup_html = '<div><h2>Signup Form</h2><?php echo $signup_form_html ?><input type="button" id="activate" class="button button-primary updater-act-btn updater-act-btn-align" style="margin-top: 15px;" value="<?php echo __('Sign Up'); ?>" data-process="reactivate"><span class="spinner" style="float: none; margin-top:20px; margin-left:10px"></span></div>';
		...
		jQuery.ajax(
			{
				...
				success: function(result){
					...
					if(result == "Access Granted!"){
						var buttons = '<button class="button updater-act-btn btn-dev-access" id="developer-access-revoke"><span class="dashicons dashicons-admin-network"></span><?php echo __('Revoke Access'); ?></button>';
						var html = '<p>You have granted access to developer. The developer access will be automatically revoked.</strong></p>'+buttons;
		...
		jQuery.ajax(
			{
				...
				success: function(result){
					...
					if(result == "Access Revoked!"){
						var buttons = '<button class="button updater-act-btn btn-dev-access" id="developer-access"><span class="dashicons dashicons-admin-network"></span><?php echo __('Grant Temporary Access to Developers'); ?></button>';
						jQuery(".access-buttons").html(buttons);
