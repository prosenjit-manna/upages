...
class Javo_Theme_Importer {
    ...
    public function add_admin() {
        add_submenu_page('themes.php', "Import Demo Data", "Import Demo Data", 'switch_themes', 'javo_demo_installer', array($this, 'demo_installer'));
    }
    ...
    public function demo_installer() {
        ...
        <form method="post">
            <input type="hidden" name="demononce" value="<?php echo wp_create_nonce('javo-demo-code'); ?>" />
        ...
        if( 'demo-data' == $action && check_admin_referer('javo-demo-code' , 'demononce')){
    ...
    public function set_demo_theme_options( $file ) {
		...
		if ( ! file_exists( $file ) ) {
			wp_die(
				__( 'Theme options Import file could not be found. Please try again.', 'javo_fr' ),
				'',
		...
		if ( empty( $data ) ) {
			wp_die(
				__( 'Theme options import data could not be read. Please try a different file.', 'javo_fr' ),
				'',
				array( 'back_link' => true )
			);
		}
		...
		// Hook before import
		$data = apply_filters( 'javo_theme_import_theme_options', $data );
		//echo "ok222=".$data;
    ...
    function available_widgets() {
    	...
    	return apply_filters( 'javo_theme_import_widget_available_widgets', $available_widgets );
     ...
     */
    public function process_widget_import_file( $file ) {

    	// File exists?
    	if ( ! file_exists( $file ) ) {
    		wp_die(
    			__( 'Widget Import file could not be found. Please try again.', 'javo_fr' ),
    			'',
    			array( 'back_link' => true )
    		);
    	}
    ...
    public function import_widgets( $data ) {
    	...
    	if ( empty( $data ) || ! is_object( $data ) ) {
    		wp_die(
    			__( 'Widget import data could not be read. Please try a different file.', 'javo_fr' ),
    			'',
    			array( 'back_link' => true )
    		);
    	}
    	...
    	// Hook before import
    	$data = apply_filters( 'javo_theme_import_widget_data', $data );
    	...
    	foreach ( $data as $sidebar_id => $widgets ) {
    		...
    		if ( isset( $wp_registered_sidebars[$sidebar_id] ) ) {
    			...
    			$sidebar_message = __( 'Sidebar does not exist in theme (using Inactive)', 'javo_fr' );
    		}
    		...
    		foreach ( $widgets as $widget_instance_id => $widget ) {
    			...
    			if ( ! $fail && ! isset( $available_widgets[$id_base] ) ) {
    				...
    				$widget_message = __( 'Site does not support widget', 'javo_fr' ); // explain why widget not imported
    			}

    			// Filter to modify settings before import
    			// Do before identical check because changes may make it identical to end result (such as URL replacements)
    			$widget = apply_filters( 'javo_theme_import_widget_settings', $widget );
    			...
    			if ( ! $fail && isset( $widget_instances[$id_base] ) ) {
    				...
    				foreach ( $single_widget_instances as $check_id => $check_widget ) {
    					...
    					if ( in_array( "$id_base-$check_id", $sidebar_widgets ) && (array) $widget == $check_widget ) {
    						...
    						$widget_message = __( 'Widget already exists', 'javo_fr' ); // explain why widget not imported
    			...
    			if ( ! $fail ) {
    				...
    				if ( $sidebar_available ) {
    					...
    					$widget_message = __( 'Imported', 'javo_fr' );
    				} else {
    					$widget_message_type = 'warning';
    					$widget_message = __( 'Imported to Inactive', 'javo_fr' );
    				}
    			...
    			$results[$sidebar_id]['widgets'][$widget_instance_id]['title'] = $widget->title ? $widget->title : __( 'No Title', 'javo_fr' ); // show "No Title" if widget instance is untitled
    	...
    	// Hook after import
    	do_action( 'javo_theme_import_widget_after_import' );

    	// Return results
    	return apply_filters( 'javo_theme_import_widget_results', $results );
